...
/** Arm only while one iterator `next()` is outstanding; the first demand may use distinct timing and classification. */
...
| Arm and rearm only around outstanding iterator demand, including distinct first-demand timing and out-of-band activity | `dsh-timeout` (`idleWatchdog`) |
...
- **LLM adapters** — `dsh-llm-deepseek` and `dsh-llm-pi-ai` wrap actual transport iteration with `idleWatchdog`. The configured interval covers only outstanding provider demand, not time the downstream consumer spends between chunks. The pi-ai adapter may give the first Harness stream chunk a longer bound before switching to its five-minute-default idle interval. The direct DeepSeek adapter also pulses an outstanding demand when its SSE parser observes a comment, without yielding the comment as a `StreamChunk` or writing it to the session log. The pi-ai SDK does not expose comment activity to its adapter, so that path can rearm only when the SDK yields. The stable signal reaches `fetch` or the SDK for the whole call, so timeout closes the underlying request and maps to `TIMEOUT`, while an earlier caller abort maps to `ABORTED`.
...
- Model streams share one rearmable timer contract without turning a sliding idle interval into a total-call deadline or charging consumer think time. A caller may classify the first unresolved demand separately without replacing the signal the transport observes. Adapters that can observe out-of-band transport activity may pulse an outstanding demand; suppressed activity remains invisible to the watchdog. The primitive still only notifies; adapter tests prove their transports observe its stable signal and terminate.

...
| `idleWatchdog(upstream, timeoutMs, code, first?)` | Keep one stable fused signal and arm only while its guarded async-iterator `next()` is outstanding. `first` may supply a distinct `{ timeoutMs, code }`; resolution disarms; later demand or `pulse()` activity rearms; disposal clears; concurrent demand rejects. |

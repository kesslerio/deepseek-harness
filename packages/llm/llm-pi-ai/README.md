...
The two `http*TimeoutMs` keys are process-wide, not per route: Node's fetch is its built-in undici, which aborts any exchange whose response headers or body stall for 300,000 ms, and a streaming endpoint that prefills for minutes emits headers immediately and then silence, so those defaults kill the request mid-flight before the route's `firstEventTimeoutMs` watchdog can act. At mount this plugin installs the shared fetch dispatcher carrying these guards (`0` disables a guard — the default, returning timeout ownership to the watchdogs; a finite value sets a dispatcher-level floor below them), and a settings change reinstalls it when the values differ. The scope is process-wide because pi-ai constructs its provider clients without a fetch seam; non-LLM fetchers are unaffected in practice because they own their budgets through abort signals and timeout policies.
...
- **The egress guards are process-wide, not per route** — pi-ai constructs its OpenAI clients without a fetch seam (`new OpenAI({ apiKey, baseURL })`), so `httpBodyTimeoutMs`/`httpHeadersTimeoutMs` install one shared fetch dispatcher for the whole process instead of attaching per provider request. Non-LLM fetchers bound themselves through abort signals and timeout policies, but a future consumer relying on undici's built-in 300,000 ms defaults would silently lose that backstop; the OpenAI client already supports `fetchOptions`, so a pi-ai passthrough would let these become per-profile fields. The guards also yield to `@deepseek-ai/dsh-http-proxy` when one of its policies is active: that package owns undici's global dispatcher for proxy routing, so this adapter installs nothing then and the proxy dispatcher keeps its default timeouts — a coordination gap, short of threading both into one dispatcher.

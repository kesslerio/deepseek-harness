...
- **The egress guards are process-wide, not per route** — pi-ai constructs its OpenAI clients without a fetch seam (`new OpenAI({ apiKey, baseURL })`), so `httpBodyTimeoutMs`/`httpHeadersTimeoutMs` install one shared fetch dispatcher for the whole process instead of attaching per provider request. Non-LLM fetchers bound themselves through abort signals and timeout policies, but a future consumer relying on undici's built-in 300,000 ms defaults would silently lose that backstop; the OpenAI client already supports `fetchOptions`, so a pi-ai passthrough would let these become per-profile fields.
